...
sequence_length_num = 960
# Task-specific flags
tf.app.flags.DEFINE_integer('validation_batches', 75, "How many batches to use for validation metrics.")
...
def train():
    data_list = load_data('copy', seq_len=sequence_length_num)
    ...
    ground_truth = tf.placeholder(tf.float32, [None, None, OUTPUT_SIZE])  # (batch, out)
    ...
    flat_out = tf.reshape(rnn_outputs.h, (-1, rnn_outputs.h.get_shape()[-1]))
    out = linear(flat_out, OUTPUT_SIZE, scope='output_mapping')
    out = tf.reshape(out, (tf.shape(rnn_outputs.h)[0], tf.shape(rnn_outputs.h)[1], OUTPUT_SIZE))
    ...
    flat_out = tf.reshape(out, (-1, tf.shape(out)[-1]))
    flat_y = tf.reshape(ground_truth, (-1, tf.shape(ground_truth)[-1]))
    ...
    loss_nowd = tf.reduce_mean(sample_cross_entropy)
    ...
    for e in range(EPECHS):
        ...
        minibatch_indices4test = get_minibatches_indices(
            len(x_test), FLAGS.batch_size)
        valid_error, valid_steps = 0., 0.
        for p_num, p_indices in enumerate(minibatch_indices4test):
            x = [x_test[i] for i in p_indices]
            y = y_test[p_indices]
            x, seq_lengths = pad_seqs(x)

            valid_iter_error = sess.run(
                loss_nowd,    # [mse, updated_states],
                feed_dict={
                    samples: x,
                    ground_truth: y})
