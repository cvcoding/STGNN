...
tf.app.flags.DEFINE_integer('sequence_length', 120, "Sequence length.")
...
def generate_example(seq_length, min_val, max_val):
    ...
    b2 = random.randint(0, int(seq_length * FIRST_MARKER / 100.) - 1) + seq_length//2
...
def train():
    data_list = load_data('add', seq_len=120)
    ...
    while True:
        ...
        print("epech: %f,"
              "validation error: %.7f, "
                "validation samples: %.2f%%" % (num_iters, valid_error,
