...
_concat = rnn_cell_impl_local._concat
...
def _is_keras_rnn_cell(rnn_cell):
  ...
  return (not isinstance(rnn_cell, rnn_cell_impl_local.RNNCell)
...
def dynamic_rnn(cell, inputs, sequence_length=None, initial_state=None,
  ...
  """
  rnn_cell_impl_local.assert_like_rnncell("cell", cell)
...
def _dynamic_rnn_loop(cell,
  ...
  def _time_step(time, output_ta_t, state):
    ...
    if is_keras_rnn_cell and not nest.is_sequence(state):
      state = [state]
    call_cell = lambda: cell(input_tdata, state, input_t_gate)
  ...
  _, output_final_ta, final_state = control_flow_ops.while_loop(
      cond=lambda time, *_: time < loop_bound-1,
      body=_time_step,
  ...
  **NOTE: This method is still in testing, and the API may change.**
  ...
  Raises:
    TypeError: If `cell` is not an instance of RNNCell, or `loop_fn` is not
      a `callable`.
  """
  rnn_cell_impl_local_scd.assert_like_rnncell("cell", cell)
